...
def needleman(exp_profile, dbdata):
    z1 = 0
    z2 = 0

    dbsize = dbdata.shape[0]

    nums1 = np.zeros((dbsize, 1))
    nums2 = np.zeros((dbsize, 1))

    sep = 66                    # 146 for combined, 66 for use_prof

    # href = dbdata[0, sep]
    href = 0.119071
    fref = emod * (href ** 2)

    dbdata[:, 0:sep] = dbdata[:, 0:sep] / href
    ...
    store_y = dbdata[ind1, -2]
    store_n = dbdata[ind1, -1]
...
def gpr(traindata, exp_profile, trainsize, do_x=0, do_y=0, *args):
    ...
    if do_x == 1:
        ...
        x_train[:, 0:sep] = x_train[:, 0:sep]/href
    else:
        None
...
if __name__ == '__main__':
    ...
    for it in testrow:
        ...
        sy_temp, n_temp = needleman(x_test, dataneedle)
